...
egypt_data <- egypt_bank_lending_rate %>%
  full_join(egypt_broad_money) %>%
  full_join(egypt_account_balance_pc) %>%
  full_join(egypt_account_balance_gdp) %>%
  full_join(egypt_fer_pc) %>%
  full_join(egypt_fer_gdp) %>%
  full_join(egypt_policy_rate) %>%
  full_join(egypt_portfolio_flows) %>%
  full_join(egypt_risk_premium) %>%
  full_join(egypt_volatility) %>%
  full_join(egypt_stock_price_mom) %>%
  full_join(egypt_stock_price_24mma) %>%
  full_join(egypt_term_premium) %>%
  full_join(egypt_money_velocity) %>%
