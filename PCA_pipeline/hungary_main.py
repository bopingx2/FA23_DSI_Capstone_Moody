...
HUN_df = pd.read_csv('PCA_pipeline/IMF_FCI_Hungary.csv')
...
def PCA_module(df: pd.DataFrame):
    ...
    df_combined = df_transformed.merge(HUN_df, how = 'left',on ="Date")
    end_index = None
    ...
    for i in range(len(df_PCA_decomp.columns)):
        ...
        if (pca.explained_variance_ratio_[i] > 0.3 and ((eigenvector < 0).all() or (eigenvector > 0).all())):
            ...
            with open("./PCA_pipeline/HUN_corr/"+decomp_id+".txt", "w") as f:
                print("-"*20, file = f)
            ...
            plt.savefig("./PCA_pipeline/HUN_fig/"+decomp_id+".jpg")
            plt.close()
...
if __name__ == "__main__":
    ## empty outputs
    if os.path.exists("./PCA_pipeline/HUN_corr"):
        shutil.rmtree("./PCA_pipeline/HUN_corr")
    if os.path.exists("./PCA_pipeline/HUN_fig"):
        shutil.rmtree("./PCA_pipeline/HUN_fig")
    os.mkdir("./PCA_pipeline/HUN_fig")
    os.mkdir("./PCA_pipeline/HUN_corr")
    ##

    country_df = pd.read_csv(base_dir + 'Hungary_DataFrame.csv')
    country_comb = generate_combination('Hungary')
